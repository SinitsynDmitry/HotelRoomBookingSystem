...
{
    public class RoomsRepository : IRoomsAdminRepository
    {
        ...
        {
            var newRoom = roomDto.AsOriginal();
        ...
        public async Task<int> DeleteRoomAsync(int id)
        {
            ...
            var bookings = await _context.Bookings.Where(b => b.Room.Id == id).ToListAsync();
            if (bookings?.Count>0)
            {
        ...
        {
            return await Task.FromResult(_context.Rooms.Any(e => e.Id == id));
        }
